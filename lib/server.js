...
export class ContextServer {
  ...
  async start(customTransport) {
    ...
    // Log startup info to stderr (so it doesn't interfere with stdio MCP communication)
    if (!customTransport) {
      const providerNames = this.providers.map(p => p.name || 'Unknown').join(", ");
      console.error(`\n🚀 [${this.serverName}] is ready and listening on stdio.`);
      console.error(`📦 Loaded Providers: ${providerNames || 'None'}\n`);
    }
  }
}
